...
4. **Staged implementation (checkpoint 3):** Deliver for Windows: live webcam hand detection; smooth cursor movement with calibration and sensitivity settings; reliable gestures for left-click, right-click, drag/drop, scrolling, and pause/resume; debouncing/state-machine logic to prevent accidental actions; camera selection and clear visual status feedback; emergency stop and safe recovery from camera/tracking failures; automated tests using synthetic landmarks that require neither a camera nor desktop control; Windows installation, execution, troubleshooting, and packaging instructions; a distributable Windows build when technically practical.
5. Work milestone by milestone, starting with the smallest end-to-end Windows vertical slice; keep the project runnable after each merged milestone.
6. Use reasonable defaults, but make gesture mappings configurable. For hardware-dependent behavior, add automated non-hardware tests plus a precise manual webcam validation checklist.
7. Document Android system-pointer feasibility: permissions, accessibility implications, Play Store constraints, reusable components, and the recommended next phase - do not implement it.
